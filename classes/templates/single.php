...
<div id="wpdsr-dashboard" class="wpdsr-ui wpdsr-dashboard" data-page="single">
    ...
    <main class="wpdsr-content">
			...
			<div class="wpdsr-header">
				<div class="wpdsr-header-title">
					<h2 class="title"><?php esc_html_e('Add single reviews', 'wpdsr'); ?></h2>
					<h5><?php esc_html_e('Add review to single or multiple products in one click', 'wpdsr'); ?></h5>
				</div>
			</div>

			<div class="wpdsr-content-wrapper">

				<form id="wpdsr-form-single" class="wpdsr-filtered wpdsr-form wpdsr-show-all">
					<input type="hidden" name="action" value="wpdsr_add_single"/>
					<?php
						...
						foreach( $this->settings as $setting_key => $setting ){
							...
							if( 'heading' == $setting['type'] ){
								...
								echo '<div data-id="'. esc_attr($setting['id']) .'" data-group="'. esc_attr($setting['group']) .'" data-search="'. esc_attr($setting['id']) .'" class="wpdsr-row '. esc_attr($setting['class'] ?? '') .'">';
					...
					<a class="wpdsr-button primary wpdsr-save-single" href="#"><span><?php esc_html_e('Publish review', 'wpdsr'); ?></span></a>
